<template>
  <div
    id="app-drawer"
    class="bg-primaryLight pt-6 pb-12 flex flex-col justify-between lg:z-2 md:z-2 lg:fixed md:fixed absolute lg:left-0 md:left-0 top-0 h-screen overflow-auto w-[300px]"
  >
    <div>
      ...
      <ul class="p-0 flex flex-col gap-3 mt-6">
        <li v-for="(item, idx) in menu" :key="idx">
          <div class="menu-item">
            <span
              class="menu-item-link text-dark flex justify-between items-center w-full rounded-tr-full rounded-br-full"
              :role="item.header ? '' : 'button'"
              @click="item.hasChildren ? openSubMenu(item) : goToLink(item)"
              :class="{ 'text-primary bg-primary/20 font-bold': item.parent === routeParent }"
            >
              <div class="flex gap-2 items-center">
                <i-icon :icon="item.icon" v-if="item.icon" class="menu-item-icon" />
                <span class="flex flex-column capitalize">
                  <span class="font-regular" :class="{ 'font-semibold': item.parent === routeParent }">
                    {{ item.title }}
                  </span>
                </span>
              </div>
            ...
            <div class="flex flex-col ml-16 gap-3 mt-2" v-if="subMenu === item.parent">
              <span
                role="button"
                class=""
                v-for="(data, idx) in item.children"
                :key="idx"
                @click="$router.push(data.to)"
              >
                <span
